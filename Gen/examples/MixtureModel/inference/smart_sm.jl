...
@kernel function k_targetted_splitmerge(trₜ₋₁, data, n_split_proposals)
    ...
    # Return.
    if !isnothing(two_split_clusters[1])
        @assert  i in two_split_clusters[1]
    end
    cluster_l_would_merge_into = two_split_clusters[2]
    return (
        choicemap((:partition, Πₜ²)),
        l_constraint_choicemap(Πₜ², i, k_split_choices, split_rev_rand, move_idx, cₜ¹, cluster_l_would_merge_into, Πₜ², n_split_proposals)
    )
end
@kernel function l_targetted_splitmerge(trₜ, data, n_split_proposals)
    ...
    if move_idx == 2
        ...
        c_ = in_first ? c1 : c2 # c_ is the cluster in trₜ₋₁ which i was put into, not including i
    else
        in_first = nothing
        if move_idx == 1
    ...
    @assert c_ in Πₜ₋₁¹ "Πₜ₋₁¹ = $Πₜ₋₁¹ ; c_ = $c_"

    # Sample splitting randomness for K.
    k_tried_split = length(c_) >= 2
    if k_tried_split
        trace_k_tries_to_split = update(trₜ, choicemap((:partition, add_to_cluster(Πₜ₋₁¹, c_, i))))[1]
    ...
    return (
        choicemap((:partition, Πₜ₋₁¹)),
        k_constraint_choicemap(trₜ, trₜ₋₁, i, two_split_clusters, l_split_choicemap, split_rev_rand, move_idx, in_first, n_split_proposals, cₜ₋₁²)
    )
end
function k_constraint_choicemap(trₜ, trₜ₋₁, i, two_split_clusters, l_split_choicemap, split_rev_rand_cm, l_move_idx, in_first, n_split_proposals, cₜ₋₁²)
    cm = choicemap()

    # IDX for Loc-Opt move
    # `old_cluster` is the cluster in trₜ₋₁ which `i` got put into by the loc-opt proposal
    if l_move_idx == 1
        old_cluster = setdiff(cluster_containing(trₜ[:partition], i), Set([i]))
...
function l_constraint_choicemap(Πₜ, i, k_split_choices, split_rev_rand_cm, k_move_idx, cₜ¹, l_merged_into_cluster, post_K_partition, n_split_proposals)
    cm = choicemap()
    ...
    if !isnothing(split_rev_rand_cm)
        ...
        # Then L must not have tried splitting this.
        @assert length(cluster_containing(Πₜ, i)) == 2 # (just 1 point, plus the new datapoint)
    end
    ...
    if !isnothing(k_split_choices)
        ...
        @assert length(cₜ¹) == 2 # should contain i, not be a singleton, and have < 3 since we can't split.
    end

    # For a merge move, constrain L's choice :in_first
    if k_move_idx > 2
...
function opposite_move(move, cluster_fwd_merged_into, post_fwd_Π)
    if move == 1
        return 1
    elseif move > 2
        return 2
        ...
        # if i in union(post_fwd_Π...)
        #     post_fwd_Π = setdiff(post_fwd_Π, cluster_containing(post_fwd_Π, i))
        # end
        sorted_Π = sort_by_minidx(post_fwd_Π)
        return 2 + find(sorted_Π, cluster_fwd_merged_into)
    end
end